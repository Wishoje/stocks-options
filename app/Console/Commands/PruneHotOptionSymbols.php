...
class PruneHotOptionSymbols extends Command
{
    protected $signature = 'hot-options:prune
        {--days=120 : Keep this many recent days}
        {--batch=50000 : Rows deleted per batch}
        {--sleep-ms=50 : Sleep between batches in milliseconds}';

    protected $description = 'Delete old rows from hot_option_symbols';
    ...
    public function handle(): int
    {
        ...
        $this->info("Pruning hot_option_symbols before {$cutoff} (batch={$batch})...");

        $deleted = 0;
        do {
            $n = DB::table('hot_option_symbols')
