    ...
    public function index(Request $req)
    {
        ...
        return Cache::remember($key, $ttl, function () use (
            ...
            if ($useIntraday && $isToday) {
                ...
                foreach ($live as $r) {
                    ...
                    if ($r->option_type === 'call') {
                        ...
                        if ($r->premium_usd !== null) {
                            $liveByStrike[$k]['call_prem'] += (float) $r->premium_usd;
                        }
                    } elseif ($r->option_type === 'put') {
                        $liveByStrike[$k]['put'] += (int) $r->volume;
                        if ($r->premium_usd !== null) {
                            $liveByStrike[$k]['put_prem'] += (float) $r->premium_usd;
                        }
                    }
                }
            }
            ...
            foreach ($rows as $r) {
                ...
                if ($useIntraday && $isToday) {
                    if ($sideFilter === 'call' && ((int) ($m['call_vol'] ?? 0) <= (int) ($m['put_vol'] ?? 0))) {
                        continue;
                    }
                    if ($sideFilter === 'put' && ((int) ($m['put_vol'] ?? 0) <= (int) ($m['call_vol'] ?? 0))) {
                        continue;
                    }
                }
            ...
            $items = array_values(array_filter($items, function ($row) use ($minPremium) {
                ...
                if ($p === null) {
                    return true;
                }          // keep if we could not price it
                if ($p <= 0) {
                    return false;
                }             // hide meaningless $0 rows
                if ($minPremium > 0 && $p < $minPremium) {
                    return false;
                }
    ...
    private function estimatePremiumUSD(string $symbol, string $exp, float $strike, int $callVol, int $putVol): array
    {
        ...
        if (\Schema::hasTable('option_quotes')) {
            ...
            foreach ($rows as $q) {
                ...
                if ($q->mark !== null) {
                    $m = (float) $q->mark;
                } elseif ($q->bid !== null && $q->ask !== null) {
                    $m = ((float) $q->bid + (float) $q->ask) / 2.0;
                } elseif ($q->last !== null) {
                    $m = (float) $q->last;
                } elseif ($q->bid !== null) {
                    $m = (float) $q->bid;
                ...
                if ($m !== null) {
                    $m = max($m, 0.01);
                } // enforce a penny floor to avoid tiny premiums
                $mid[$q->option_type] = $m;
            }
        ...
        $calcMid = function ($row) {
            ...
            if ($m === null && property_exists($row, 'bid') && $row->bid !== null) {
                $m = (float) $row->bid;
            }
            if ($m === null && property_exists($row, 'ask') && $row->ask !== null) {
                $m = (float) $row->ask;
            }
