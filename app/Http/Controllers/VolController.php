...
class VolController extends Controller
{
    public function term(Request $req)
    {
        ...
        $cacheKey = app(EodCacheVersion::class)->key(
            'iv_term:v2',
    ...
    public function skew(Request $req)
    {
        ...
        return \Cache::remember($cacheKey, 86400, function () use ($symbol, $exp) {
            ...
            if (! $date) {
                return response()->json((object) ['items' => [], 'date' => null], 200);
            }
    ...
    public function skewDebug(\Illuminate\Http\Request $req)
    {
        ...
        if (! $exp) {
            ...
            foreach ($expStats as $st) {
                if ($st['n_with_iv_and_delta'] === 0) {
                    continue;
                }
                if ($st['days_from_today'] < $bestDiff) {
                    $bestDiff = $st['days_from_today'];
                    $best = $st['exp'];
                }
            }
    ...
    public function skewByBucket(\Illuminate\Http\Request $req)
    {
        ...
        foreach ($rows as $r) {
            ...
            if ($diff < $best) {
                $best = $diff;
                $pick = $r;
            } elseif ($diff === $best && $exp >= $today) {
                $pick = $r;
            } // prefer not-in-the-past
        }
