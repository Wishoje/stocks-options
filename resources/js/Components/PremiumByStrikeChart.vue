...
export default {
  ...
  computed: {
    ...
    focusedData() {
      ...
      const vals = rows.map(r => {
        const call = Number(r.premium_call ?? 0)
        const put  = Number(r.premium_put  ?? 0)
        return Math.abs(call) + Math.abs(put)
      })

      const maxAbs = Math.max(...vals)
      if (!isFinite(maxAbs) || maxAbs === 0) return rows

      // keep strikes where total premium is at least 5% of max or >= 10k
      const threshold = Math.max(maxAbs * 0.05, 10_000)
    ...
    displayPoints() {
      ...
      if (!this.autoBucket || rows.length <= this.MAX_BARS) {
        return rows.map(r => ({
          ...
          call: Number(r.premium_call ?? r.call_prem ?? 0),
          put:  Number(r.premium_put  ?? r.put_prem  ?? 0),
        }))
      }

      // Bucket mode: sum call & put premium in each bucket
      ...
      for (const r of rows) {
        ...
        buckets[idx].call += Number(r.premium_call ?? r.call_prem ?? 0)
        buckets[idx].put  += Number(r.premium_put  ?? r.put_prem  ?? 0)
      }
        ...
        datasets: [
          {
            ...
            data: callData,
            stack: 'prem',
            backgroundColor: 'rgba(52,211,153,0.9)', // green-ish (calls)
            borderRadius: 3,
            barPercentage: 0.9,
          ...
          },
          {
            ...
            data: putData,
            stack: 'prem',
            backgroundColor: 'rgba(248,113,113,0.9)', // red-ish (puts)
            borderRadius: 3,
            barPercentage: 0.9,
            categoryPercentage: 0.9,
          },
  ...
  methods: {
    ...
    snapshot() {
      ...
      link.download = `${this.snapshotName || 'flow-premium'}-${ts}.png`
